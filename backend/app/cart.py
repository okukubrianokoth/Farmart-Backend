...
from app import db
from app.models import Animal, CartItem, User
                ...
                "items": [
                    {
                        ...
                        "added_at": (
                            item.added_at.isoformat() if item.added_at else None
                        ),
                    }
...
def add_to_cart():
    try:
        ...
        cart_item = CartItem.query.filter_by(
            user_id=current_user_id, animal_id=data["animal_id"]
        ).first()
        ...
        if cart_item:
            ...
            cart_item = CartItem(
                user_id=current_user_id, animal_id=data["animal_id"], quantity=quantity
            )
